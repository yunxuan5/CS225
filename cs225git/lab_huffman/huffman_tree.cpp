                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if(mergeQueue.size() == 0){
        ...
        if(temp2->freq.getFrequency() <= temp1->freq.getFrequency()){
            mergeQueue.pop();
            return temp2;
        }
        else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() != 1){
        ...
        TreeNode *next_smallest = removeSmallest(singleQueue, mergeQueue);
        TreeNode *parent = new TreeNode(smallest->freq.getFrequency() + next_smallest->freq.getFrequency());
        parent->left = smallest;
        parent->right = next_smallest;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if(bfile.getNextBit()){
        ...
        Frequency freq(c, 0);
        new_ = new TreeNode(freq);
    }
    else{
        Frequency freq(0);
        new_ = new TreeNode(freq);
        new_->left = readTree(bfile);
        new_->right = readTree(bfile);
    }
